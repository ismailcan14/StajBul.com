...
<div class="container my-5">
    ...
    <form method="POST" action="{{ route('student.messages.send') }}" id="message-form">
        @csrf
        <input type="hidden" name="receiver_id" id="receiver-id" value="{{ $company->user_id }}">
...
$(document).ready(function() {
    ...
    // 🔁 Otomatik mesaj çekme (polling)
    setInterval(function () {
        $.ajax({
            url: "{{ route('student.messages.fetch') }}",
            type: 'POST',
            data: {
                _token: "{{ csrf_token() }}",
